 ...
 * @author pchr
 */
public class PlaneStrainQuad extends Quad{
    private static int numberOfPlaneStrainQuad = 0;
    // constructor
    public PlaneStrainQuad(){}
    
    public PlaneStrainQuad(int id, Node node1, Node node2, Node node3, Node node4,
                             Material Mater, CrossSection Sect){
        this.id=id;
        ++numberOfPlaneStrainQuad;
    ...
    public AbstractMatrix getEmat(){
        for(int i=0; i<3; i++){
            ...
            double E1=((ElasticMaterial) this.ElementMaterial).getElasticity();
            double v1=((ElasticMaterial) this.ElementMaterial).getPoisson();
            double E=E1/(1.-v1*v1);
            double v=v1/(1.-v1);
            double coef=E/(1.-v*v);
    ...
    public double getStressVonMisses(double[] at, int LC,int step){
        ...
        double s33=((ElasticMaterial)ElementMaterial).getLameLambda()*(Stress[0]+Stress[1]);
        return Math.sqrt(0.5*(
                (Stress[0]-Stress[1])*(Stress[0]-Stress[1])
                +(Stress[1]-s33)*(Stress[1]-s33)
                +(s33-Stress[0])*(s33-Stress[0])
