...
import mathman.ConstantDoubleFunction;
 ...
 * @author pchr
 */
public class shearframe  implements FirstOrderDifferentialEquations, contraption{
    ...
    public shearframe(double[] masses, double[] springs){
        num_sdofSeries++;
        id=num_sdofSeries;
        ...
        this.df=new DoubleFunction[len];
    ...
    public shearframe(double[] masses, double[] springs, double[] dashpots){
        num_sdofSeries++;
        id=num_sdofSeries;
        int len=springs.length;
        this.masses=new double[len];
    ...
    public void SelfPortrait(Graphics2D g2, double ymin, double ymax, int ye, int ys, double xmin, double xmax, int xe, int xs) {
        ...
        g2.drawLine(transX.apply(x0-dx), transY.apply(y0), transX.apply(x0-dx),  transY.apply( y0+this.totLength));
        g2.drawLine(transX.apply(x0+dx), transY.apply(y0), transX.apply(x0+dx),  transY.apply( y0+this.totLength ));
         g2.setStroke(curSTroke);
        
        double ylevel=y0;
        for(int i=0;i<this.lengths.length;i++){
            ylevel+=this.lengths[i];
            // mass
            if(i<this.masses.length){
                GeneralPath gp = new GeneralPath();
                gp.moveTo(transX.apply(x0-dx), transY.apply(ylevel+dy));
                gp.lineTo(transX.apply(x0+dx), transY.apply(ylevel+dy));
                gp.lineTo(transX.apply(x0+dx), transY.apply(ylevel-dy));
                gp.lineTo(transX.apply(x0-dx), transY.apply(ylevel-dy));
                gp.closePath();
                g2.fill(gp);
    ...
    public void Motion(Graphics2D g2, double ymin, double ymax, int ye, int ys, double xmin, double xmax, int xe, int xs, int step, double scale) {
        ...
        double[] disp= new double[this.masses.length];
        for(int i=0;i<disp.length;i++)disp[i]=this.u[i][step]*scale;
        
        double ylevel=y0;
        for(int i=0;i<this.lengths.length;i++){
            ylevel+=this.lengths[i];
            double dispb=0.0;if(i>0)dispb=disp[i-1];
            // mass
            if(i<this.masses.length){
                GeneralPath gp = new GeneralPath();
                gp.moveTo(transX.apply(x0-dx+disp[i]), transY.apply(ylevel+dy));
                gp.lineTo(transX.apply(x0+dx+disp[i]), transY.apply(ylevel+dy));
                gp.lineTo(transX.apply(x0+dx+disp[i]), transY.apply(ylevel-dy));
                gp.lineTo(transX.apply(x0-dx+disp[i]), transY.apply(ylevel-dy));
                gp.closePath();
                g2.fill(gp);
                ...
                for(int n=1;n<=split;n++){
                    double y2=ylevel;
                    double y1=y2-lengths[i];
                    double y=y2-y1;
                    double a=(disp[i]-dispb)/(y*y*y-3.0*y*y*y/2.0);
                    double b=-3.0*y*a/2.0;
                    double yn=y1+this.lengths[i]*n/split;
                    double yn0=y1+this.lengths[i]*(n-1)/split;
                    double dn=dispb+a*(yn-y1)*(yn-y1)*(yn-y1)+b*(yn-y1)*(yn-y1);
                    double dn0=dispb+a*(yn0-y1)*(yn0-y1)*(yn0-y1)+b*(yn0-y1)*(yn0-y1);
                    
                    g2.drawLine(transX.apply(x0-dx+dn0), transY.apply(yn0), transX.apply(x0-dx+dn),  transY.apply(yn));
                    g2.drawLine(transX.apply(x0+dx+dn0), transY.apply(yn0), transX.apply(x0+dx+dn),  transY.apply(yn));
                }
    ...
    public RealMatrix getStiffness(){
        double[][] matrixData2 = new double[this.springs.length][this.springs.length];
        matrixData2[0][0]=this.springs[0];
        if(this.masses.length>1){
            matrixData2[0][0]+=this.springs[1];
            ...
            matrixData2[masses.length-1][masses.length-1]=this.springs[masses.length-1];
    ...
    public RealMatrix getDamping(){
        ...
        if(this.Rayleigh){
            ...
            matrixData2 = new double[this.dashpots.length][this.dashpots.length];
            matrixData2[0][0]=this.dashpots[0];
            if(this.masses.length>1){
                matrixData2[0][0]+=this.dashpots[1];
                matrixData2[0][1]=-this.dashpots[1];
                for(int i=1;i<this.masses.length-2;i++){
                    matrixData2[i][i]=this.dashpots[i]+this.dashpots[i+1];
                    matrixData2[i][i-1]=-this.dashpots[i];
                    matrixData2[i][i+1]=-this.dashpots[i+1];
                }
                matrixData2[masses.length-1][masses.length-1]=this.dashpots[masses.length-1];
